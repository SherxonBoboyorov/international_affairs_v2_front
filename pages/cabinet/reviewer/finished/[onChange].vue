...
const fetchData = async () => {
  ...
  try {
    const { data: responseData } = await GET<ArticleAssignment>(
      `reviewer/articles/completed/${onChangeParam}`
    );
    if (responseData) {
  ...
  <LoaderBox
    ...
    <div
      ...
      <div class="page-form-body">
        ...
        <div class="bg-white large mb-40">
          <ul class="mb-20">
            <li
              v-for="(criterion, index) in data?.completed_review?.scores"
              :key="index">
            ...
            <el-row
              ...
              <el-col :span="24">
                <el-form-item prop="general_recommendation">
                  <el-select
                    v-model="data.completed_review.general_recommendation"
                    disabled
              ...
              <el-col :span="24">
                <el-form-item prop="review_comments">
                  <el-input
                    v-model="data.completed_review.review_comments"
                    disabled
              ...
              <el-col :span="24">
                <el-form-item prop="review_file">
                  <a
                    v-for="(item, index) in data?.completed_review.review_files"
                    :key="index"
                  ...
                  <a
                    v-for="(item, index) in data?.completed_review.draft_files"
                    :key="index"
                    style="margin-right: 20px; font-weight: 600; color: #880019"
                    :href="config.public.apiImgUrl + item.path">
                    {{ item.original_name }}
        ...
        <div class="bg-white large">
          ...
          <ul>
            <li v-if="data.completed_review?.status_dates?.assigned_at">
              <span>Назначено:</span>
              <span>{{
                formateDate(
                  data.completed_review?.status_dates?.assigned_at.split("T")[0]
                )
              }}</span>
            </li>
            <li v-if="data.completed_review?.status_dates?.in_progress_at">
              <span>В работе:</span>
              <span>{{
                formateDate(
                  data.completed_review?.status_dates?.in_progress_at.split(
                    "T"
                  )[0]
                )
              }}</span>
            </li>
            <li v-if="data.completed_review?.status_dates?.refused_at">
              <span>Отклонено:</span>
              <span>{{
                formateDate(
                  data.completed_review?.status_dates?.refused_at.split("T")[0]
                )
              }}</span>
            </li>
            <li v-if="data.completed_review?.status_dates?.completed_at">
              <span>Завершено:</span>
              <span>{{
                formateDate(
                  data.completed_review?.status_dates?.completed_at.split(
                    "T"
                  )[0]
                )
              }}</span>
            </li>
            <li v-if="data.completed_review?.status_dates?.status_changed_at">
              <span>Изменено:</span>
              <span>{{
                formateDate(
                  data.completed_review?.status_dates?.status_changed_at.split(
                    "T"
                  )[0]
                )
              }}</span>
            </li>
            <li v-if="data.completed_review?.status_dates?.deadline">
              <span>Срок:</span>
              <span>{{
                formateDate(
                  data.completed_review?.status_dates?.deadline.split("T")[0]
                )
              }}</span>
            </li>
            <li v-if="data.completed_review?.status_dates?.extension_date">
              <span>Изменить срок:</span>
              <span>{{
                formateDate(
                  data.completed_review?.status_dates?.extension_date.split(
                    "T"
                  )[0]
                )
              }}</span>
