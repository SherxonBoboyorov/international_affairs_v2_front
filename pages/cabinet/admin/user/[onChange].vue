...
const submitForm = async (formEl) => {
  ...
  if (isEdit) {
    status.value = await PUT(`admin/user/${route.params.onChange}`, payload);
  } else {
    status.value = await POST(`admin/user`, payload);
  }
  ...
  if (status.value) {
    ...
    router.push("/cabinet/admin/user");
  }
};
...
onMounted(async () => {
  ...
  if (isEdit) {
    const userRes = await GET(`admin/user/${route.params.onChange}`);
  ...
  console.log({
    ...
    jobTitleList: jobTitleList.value,
    roleList: roleList.value,
    routeParam: route.params.onChange,
  });
